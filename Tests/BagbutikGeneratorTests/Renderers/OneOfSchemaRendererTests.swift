...
final class OneOfSchemaRendererTests: XCTestCase {
    ...
    func testRenderWithSubSchemas() throws {
        // Given
        let renderer = OneOfSchemaRenderer()
        let jsonPointerSchema = ObjectSchema(name: "JsonPointer", properties: ["pointer": .simple(.init(type: "string"))])
        let parameterSchema = ObjectSchema(name: "Parameter", properties: ["parameter": .simple(.init(type: "string"))])
    ...
    func testUnknownTypeForOption() throws {
        ...
        let oneOfSchema = OneOfSchema(options: [
            .objectSchema(.init(name: "AppCategory")),
            .objectSchema(.init(name: "SomeUnknownSchema"))
        ])
        // When

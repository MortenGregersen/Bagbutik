...
/**
 ...
 Full documentation:
 <https://developer.apple.com/documentation/appstoreconnectapi/subscriptionversionsresponse>
 */
public struct SubscriptionVersionsResponse: Codable, Sendable, PagedResponse {
    public typealias Data = SubscriptionVersion
    ...
    public func getImage(for subscriptionVersion: SubscriptionVersion) -> SubscriptionImageV2? {
        included?.compactMap { relationship -> SubscriptionImageV2? in
            ...
            return image
        }.first { $0.id == subscriptionVersion.relationships?.image?.data?.id }
    }

    public func getImages(for subscriptionVersion: SubscriptionVersion) -> [SubscriptionImageV2] {
        guard let imageIds = subscriptionVersion.relationships?.images?.data?.map(\.id),
    ...
    public func getLocalizations(for subscriptionVersion: SubscriptionVersion) -> [SubscriptionLocalizationV2] {
        guard let localizationIds = subscriptionVersion.relationships?.localizations?.data?.map(\.id),
    ...
    public func getSubscription(for subscriptionVersion: SubscriptionVersion) -> Subscription? {
        included?.compactMap { relationship -> Subscription? in
            guard case let .subscription(subscription) = relationship else { return nil }
            return subscription
        }.first { $0.id == subscriptionVersion.relationships?.subscription?.data?.id }
    }

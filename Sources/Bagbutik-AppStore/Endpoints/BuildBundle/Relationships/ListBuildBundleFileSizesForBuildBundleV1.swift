    ...
    public enum Field: FieldParameter {
        ...
        public enum BuildBundleFileSizes: String, Sendable, ParameterValue, Codable, CaseIterable {
            ...
            public init(from decoder: Decoder) throws {
                ...
                if let value = BuildBundleFileSizes(rawValue: string) {
                    self = value
                } else if let value = BuildBundleFileSizes(rawValue: string.uppercased()) {
                    self = value
                } else {
                    throw DecodingError.dataCorruptedError(
                        in: container,
                        debugDescription: "Invalid BuildBundleFileSizes value: \(string)"
                    )
                }
            }
        }

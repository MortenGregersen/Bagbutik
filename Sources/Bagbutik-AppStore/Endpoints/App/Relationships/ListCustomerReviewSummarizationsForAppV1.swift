    ...
    public enum Field: FieldParameter {
        ...
        public enum CustomerReviewSummarizations: String, Sendable, ParameterValue, Codable, CaseIterable {
            ...
            public init(from decoder: Decoder) throws {
                ...
                if let value = CustomerReviewSummarizations(rawValue: string) {
                    self = value
                } else if let value = CustomerReviewSummarizations(rawValue: string.uppercased()) {
                    self = value
                } else {
                    throw DecodingError.dataCorruptedError(
                        in: container,
                        debugDescription: "Invalid CustomerReviewSummarizations value: \(string)"
                    )
                }
            }
        }

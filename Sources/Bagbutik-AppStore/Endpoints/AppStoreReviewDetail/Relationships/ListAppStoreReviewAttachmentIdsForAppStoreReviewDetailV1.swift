import Bagbutik_Core
import Bagbutik_Models

public extension Request {
    /**
     # GET /v1/appStoreReviewDetails/{id}/relationships/appStoreReviewAttachments

     Full documentation:
     <https://developer.apple.com/documentation/appstoreconnectapi/get-v1-appStoreReviewDetails-_id_-relationships-appStoreReviewAttachments>

     - Parameter id: The id of the requested resource
     - Parameter limit: Maximum resources per page - maximum 200
     - Returns: A ``Request`` to send to an instance of ``BagbutikService``
     */
    static func listAppStoreReviewAttachmentIdsForAppStoreReviewDetailV1(id: String,
                                                                         limit: Int? = nil) -> Request<AppStoreReviewDetailAppStoreReviewAttachmentsLinkagesResponse, ErrorResponse> {
        .init(
            path: "/v1/appStoreReviewDetails/\(id)/relationships/appStoreReviewAttachments",
            method: .get,
            parameters: .init(limit: limit))
    }
}

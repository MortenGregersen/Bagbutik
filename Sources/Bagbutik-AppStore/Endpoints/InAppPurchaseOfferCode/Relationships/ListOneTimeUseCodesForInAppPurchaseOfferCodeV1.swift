import Bagbutik_Core
import Bagbutik_Models

public extension Request {
    /**
     # List all one-time use codes for an in-app purchase offer code
     Get a list of one-time use codes for a specific in-app purchase offer code.

     Full documentation:
     <https://developer.apple.com/documentation/appstoreconnectapi/get-v1-inAppPurchaseOfferCodes-_id_-oneTimeUseCodes>

     - Parameter id: The id of the requested resource
     - Parameter fields: Fields to return for included related types
     - Parameter includes: Relationship data to include in the response
     - Parameter limit: Maximum resources per page - maximum 200
     - Returns: A ``Request`` to send to an instance of ``BagbutikService``
     */
    static func listOneTimeUseCodesForInAppPurchaseOfferCodeV1(id: String,
                                                               fields: [ListOneTimeUseCodesForInAppPurchaseOfferCodeV1.Field]? = nil,
                                                               includes: [ListOneTimeUseCodesForInAppPurchaseOfferCodeV1.Include]? = nil,
                                                               limit: Int? = nil) -> Request<InAppPurchaseOfferCodeOneTimeUseCodesResponse, ErrorResponse> {
        .init(
            path: "/v1/inAppPurchaseOfferCodes/\(id)/oneTimeUseCodes",
            method: .get,
            parameters: .init(
                fields: fields,
                includes: includes,
                limit: limit))
    }
}

public enum ListOneTimeUseCodesForInAppPurchaseOfferCodeV1 {
    /**
     Fields to return for included related types.
     */
    public enum Field: FieldParameter {
        /// The fields to include for returned resources of type actors
        case actors([Actors])
        /// The fields to include for returned resources of type inAppPurchaseOfferCodeOneTimeUseCodes
        case inAppPurchaseOfferCodeOneTimeUseCodes([InAppPurchaseOfferCodeOneTimeUseCodes])

        public enum Actors: String, Sendable, ParameterValue, Codable, CaseIterable {
            case actorType
            case apiKeyId
            case userEmail
            case userFirstName
            case userLastName

            public init(from decoder: Decoder) throws {
                let container = try decoder.singleValueContainer()
                let string = try container.decode(String.self)
                if let value = Actors(rawValue: string) {
                    self = value
                } else if let value = Actors(rawValue: string.uppercased()) {
                    self = value
                } else {
                    throw DecodingError.dataCorruptedError(
                        in: container,
                        debugDescription: "Invalid Actors value: \(string)"
                    )
                }
            }
        }

        public enum InAppPurchaseOfferCodeOneTimeUseCodes: String, Sendable, ParameterValue, Codable, CaseIterable {
            case active
            case createdByActor
            case createdDate
            case deactivatedByActor
            case environment
            case expirationDate
            case numberOfCodes
            case values

            public init(from decoder: Decoder) throws {
                let container = try decoder.singleValueContainer()
                let string = try container.decode(String.self)
                if let value = InAppPurchaseOfferCodeOneTimeUseCodes(rawValue: string) {
                    self = value
                } else if let value = InAppPurchaseOfferCodeOneTimeUseCodes(rawValue: string.uppercased()) {
                    self = value
                } else {
                    throw DecodingError.dataCorruptedError(
                        in: container,
                        debugDescription: "Invalid InAppPurchaseOfferCodeOneTimeUseCodes value: \(string)"
                    )
                }
            }
        }
    }

    /**
     Relationship data to include in the response.
     */
    public enum Include: String, IncludeParameter, CaseIterable {
        case createdByActor
        case deactivatedByActor
    }
}
